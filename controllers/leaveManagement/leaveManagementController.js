...
const addLeave = async (req, res) => {
    try {
        ...
        if (overlappingLeave.length > 0) {
            return res.status(400).json({ message: "Leave request overlaps with an existing leave entry. Please check your leave schedule." });
        }
        ...
        if (requestedDays > remainingLeaveDays) {
            ...
            return res.status(400).json({ message: `Not enough leave days. You are requesting ${extraDays} extra days. Please connect with HR.` });
        }
        ...
        return res.status(201).json({ message: "Leave added successfully", reason: leave_reason, leave_id: newLeave._id });
    } catch (error) {
        console.error(error);
...
const processLeave = async (req, res) => {
    try {
        ...
        if (leave.leaveProcessed) {
            return res.status(409).json({ message: `Leave request already processed - ${leave.leave_status}` });
        }
        ...
        if (action === 'approve') {
            ...
            leave.processedBy = loggedInuser;
            await leave.save();
            return res.status(200).json({ message: `Leave request ${action === 'reject' ? 'rejected' : 'cancelled'} successfully` });
        } else {
            return res.status(400).json({ message: "Invalid action. Use 'approve', 'reject', or 'cancel'." });
        }
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: "Internal server error" });
...
const getSubordinateLeaveRequests = async (req, res) => {
    try {
        ...
        const team = await teamModel.findOne({ reportingManager: reportingManager._id }).populate('members');

        if (!team) {
            return res.status(404).json({ message: "No team found for this reporting manager" });
        }
