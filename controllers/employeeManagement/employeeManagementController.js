...
const updateEmployee = async (req, res) => {
  ...
  try {
    ...
    if ((updateFields.password && updateFields.password !== currentEmployee.password) || (updateFields.password && updateFields.password === currentEmployee.password)) {
      ...
      currentEmployee.password = await bcrypt.hash(currentEmployee.password, 10);
    }
...
const getParticularEmployee = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Employee queried successfully', data: user });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
