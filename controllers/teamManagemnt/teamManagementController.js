...
const addMembersToTeam = async (req, res) => {
    try {
        ...
        if (members.length !== memberIds.length) {
            return res.status(400).json({ message: 'Some members are not part of the company, kindly check' });
        }
        ...
        res.json({ message: 'Members added successfully', team, teamLeader, membersDetails });
    } catch (error) {
...
const removeMembersFromTeam = async (req, res) => {
    try {
        ...
        team.members = team.members.filter(member => !memberIds.includes(member.toString()));
        await team.save();
        ...
        res.json({ message: 'Members removed successfully', team, remainingMembersDetails });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
...
const getParticularTeamDetails = async (req, res) => {
    try {
        ...
        res.json({ teamName, teamReportingManager: managerDetails, teamMembers: membersDetails });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
...
const fetchYourOwnTeam = async (req, res) => {
    try {
        ...
        for (const team of teams) {
            ...
            teamDetails.push({ teamName, teamReportingManager: managerDetails, teamMembers: membersDetails });
        }
